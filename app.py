...
def increment_visit_count(filename):
    ...
    # Increment the visit count or add a new entry
    if filename in visit_counts:
        visit_counts[filename] += 1
    else:
        visit_counts[filename] = 1

    # Reorder visit_counts to move the recently visited site to the top
    ordered_visit_counts = {filename: visit_counts[filename]}  # Start with the updated site
    for site, count in visit_counts.items():
        if site != filename:  # Skip the recently updated site
            ordered_visit_counts[site] = count

    # Save the ordered visit counts
    save_visit_counts(ordered_visit_counts)
...
def remove_href_from_file(html_file):
    ...
    for a_tag in soup.find_all('a', href=True):
        a_tag['href'] = '#'
        # Remove target attributes to prevent opening in new tabs
